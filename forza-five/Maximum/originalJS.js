...
const oneasHud = {
    ...
    menuSettings: {
        defaultData: {
            color: "first-color", // является дефолтным значением
        ...
        removeClassList(color) {
            document.body.classList.remove(color);
        },

        changeHudColor(color) {
            if (this.data.prevColor) this.removeClassList(this.data.prevColor);

            this.addClassList(color);
        ...
        addMenuOption() {
            jsLoader.mainMenu.addNewOption(3, "[Сборка] Цвет интерфейсов", {
                data: [
                    {
                        id: "first-color",
                        value: "Голубой",
